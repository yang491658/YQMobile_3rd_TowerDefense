...
public class SpeedUp : TowerSkill
{
    ...
    public override void OnUpdate(Tower _tower, Monster _target, float _deltaTime)
    {
        ...
        for (int i = targets.Count - 1; i >= 0; i--)
        {
            ...
            target.Buff.RemoveStat(_tower, TowerBuff.SubType.Speed);
        ...
        {
            target.Buff.ApplyStat(_tower, TowerBuff.SubType.Speed, factor, 0f, TowerBuff.ApplyType.Refresh);
    ...
    {
        for (int i = targets.Count - 1; i >= 0; i--)
            targets[i].Buff.RemoveStat(_tower, TowerBuff.SubType.Speed);
